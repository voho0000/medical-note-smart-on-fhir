...
export function HfDiagnosisConfirmation({ answer, onConfirm, now, isEnglish, basis, followUp }: {
  ...
  return <div className="space-y-2 px-4 py-3" data-testid="cdss-diagnosis-confirmation">
    ...
    {confirmation ? <p className="break-words text-xs text-muted-foreground">{confirmation.method === 'existing' ? (isEnglish ? 'Existing diagnosis adopted' : '沿用既有確診') : (isEnglish ? 'Confirmed this visit' : '本次確認診斷')} · {new Date(confirmation.confirmedAt).toLocaleString(isEnglish ? 'en-US' : 'zh-TW')}<br />{confirmation.basis}</p> : onConfirm ? <Button variant="outline" className="h-auto min-h-11 whitespace-normal" onClick={() => setOpen(true)}>{followUp ? (isEnglish ? 'Record diagnosis confirmation' : '補記診斷確認紀錄') : (isEnglish ? 'Review and confirm diagnosis' : '檢視並確認診斷')}</Button> : null}
    {confirmation && onConfirm ? <Button variant="outline" className="min-h-11" onClick={() => { setMethod(confirmation.method); setNote(confirmation.basis); setOpen(true) }}>{isEnglish ? 'Edit diagnosis confirmation' : '修改診斷確認紀錄'}</Button> : null}
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogContent>
        <DialogTitle>{isEnglish ? 'Confirm heart-failure diagnosis' : '確認心衰竭診斷'}</DialogTitle>
        <DialogDescription>{isEnglish ? 'Diagnostic evidence remains open and editable after confirmation. Select Follow-up when ready.' : '確認後診斷依據仍保持開啟，可繼續修改；完成後可切換至追蹤。'}</DialogDescription>
        ...
        <DialogFooter>
          ...
          <Button onClick={() => { onConfirm?.({ ...answer, hfSuspicion: 'suspected', answeredOn: todayIsoDate(now), diagnosisConfirmation: { method, confirmedAt: new Date().toISOString(), basis: confirmation ? note.trim() || basis : [basis, note.trim()].filter(Boolean).join(' · ') } }); setOpen(false) }}>{confirmation ? (isEnglish ? 'Save changes' : '儲存修改') : (isEnglish ? 'Confirm diagnosis' : '確認診斷')}</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  </div>
