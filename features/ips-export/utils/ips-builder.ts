...
import { escapeHtml, makeEntry, orphanResultObservations, uuidv4 } from './ips-helpers'
import {
  buildPatient,
...
export interface IpsSectionLabels {
  ...
  sourceNotice: string
  notMedicalRecordNotice: string
  noClinicalAttestationNotice: string
...
export const DEFAULT_SECTION_LABELS: IpsSectionLabels = {
  ...
  notMedicalRecordNotice: 'This export is a user-held health-data summary, not an original medical record issued by a healthcare institution.',
...
export interface BuildIpsOptions {
  ...
  /** Include direct patient identifiers. Default true for person-mediated sharing. */
  includePatientIdentifiers?: boolean
}

function loincCode(code: string, display: string): FhirCodeableConcept {
  return { coding: [{ system: SYSTEM.loinc, code, ...(display ? { display } : {}) }] }
}
...
function assembleSection(
  ...
  const section: IpsCompositionSection = {
    title,
    // Keep the human-facing section title localized, but omit Coding.display:
    // it is terminology-controlled and a translated/custom title is not a
    // valid LOINC display. The code itself is sufficient and validates cleanly.
    code: loincCode(loinc, ''),
    text: {
      status: 'generated',
...
export function buildIpsBundle(opts: BuildIpsOptions): IpsBundle {
  ...
  const includePatientIdentifiers = opts.includePatientIdentifiers !== false
  const patientEntry = buildPatient(opts.patient, {
    includeIdentifiers: includePatientIdentifiers,
    maskedLabel: labels.maskedPatient,
  })
  ...
  const composition: FhirResource = {
    ...
    title: labels.documentTitle || IPS_DOC_TITLE,
    confidentiality: 'R',
    text: {
      status: 'generated',
      div: `<div xmlns="http://www.w3.org/1999/xhtml"><p><strong>${escapeHtml(labels.sourceNotice)}</strong></p><p>${escapeHtml(labels.notMedicalRecordNotice)}</p><p>${escapeHtml(labels.noClinicalAttestationNotice)}</p></div>`,
    },
    section: sections,
  }
  ...
  const bundle: IpsBundle = {
    ...
    meta: {
      ...
      tag: [{
        system: 'https://mediprisma.app/fhir/CodeSystem/document-origin',
        code: 'patient-mediated-health-bank-export',
        display: 'Patient-mediated export from Taiwan NHI My Health Bank data',
      }],
    },
    type: 'document',
    identifier: {
