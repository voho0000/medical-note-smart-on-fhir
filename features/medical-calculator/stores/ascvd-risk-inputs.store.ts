/** Physician-entered ASCVD calculator criteria, encrypted per patient for this browser tab. */
...
export interface AscvdRiskInputEntry {
...
export interface AscvdRiskInputs {
  /** Keyed by the calculator input key (`vhr:high:smoking`, …). */
  entries: Readonly<Record<string, AscvdRiskInputEntry>>
}

export const EMPTY_ASCVD_RISK_INPUTS: AscvdRiskInputs = Object.freeze({ entries: Object.freeze({}) })
 ...
 */
export type AscvdRiskInputsPatch = Readonly<Record<string, { value: string; measuredOn?: string } | null>>
 ...
 */
export function mergeAscvdRiskInputs(
  current: AscvdRiskInputs | undefined,
  patch: AscvdRiskInputsPatch,
  now: Date = new Date(),
): AscvdRiskInputs {
  const base = current ?? EMPTY_ASCVD_RISK_INPUTS
  const modifiedAt = now.toISOString()
  const entries: Record<string, AscvdRiskInputEntry> = { ...base.entries }
  let changed = false
...
const STORAGE_PREFIX = 'calculator-ascvd-risk-inputs:'

/** The key one patient's encrypted typed ASCVD criteria are kept under. */
export function ascvdRiskInputsStorageKey(patientId: string): string {
 ...
 */
function toAscvdRiskInputs(parsed: unknown): AscvdRiskInputs {
  try {
    if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed)) return EMPTY_ASCVD_RISK_INPUTS
    const record = (parsed as Record<string, unknown>).entries
    if (!record || typeof record !== 'object') return EMPTY_ASCVD_RISK_INPUTS
    const entries: Record<string, AscvdRiskInputEntry> = {}
    ...
    return EMPTY_ASCVD_RISK_INPUTS
  }
}

function writeStored(patientId: string, inputs: AscvdRiskInputs): void {
  const key = ascvdRiskInputsStorageKey(patientId)
...
interface AscvdRiskInputsState {
  byPatientId: Readonly<Record<string, AscvdRiskInputs>>
  ...
  setInputs: (patientId: string, patch: AscvdRiskInputsPatch, now?: Date) => void
  clearInputs: (patientId: string) => void
}

export const useAscvdRiskInputsStore = create<AscvdRiskInputsState>()((set, get) => ({
  byPatientId: {},
  hydratedPatientIds: {},
  ...
  hydrate: (patientId) => {
    ...
    // writes anything.
    if (state.byPatientId[patientId] || !hasEncryptedAnswers(ascvdRiskInputsStorageKey(patientId))) {
      set((current) => ({
        byPatientId: current.byPatientId[patientId]
          ? current.byPatientId
          : { ...current.byPatientId, [patientId]: EMPTY_ASCVD_RISK_INPUTS },
    ...
    const apply = (inputs: AscvdRiskInputs) => {
      if (!settle()) return
      set((current) => ({
    ...
    void loadEncryptedAnswers<unknown>(ascvdRiskInputsStorageKey(patientId))
      .then((stored) => apply(toAscvdRiskInputs(stored)))
      // A record that cannot be read leaves every field to the report, which
      // is a first visit's reading.
      .catch(() => apply(EMPTY_ASCVD_RISK_INPUTS))
  },
  ...
  setInputs: (patientId, patch, now = new Date()) => {
    ...
    set((state) => {
      ...
      const next = mergeAscvdRiskInputs(current, patch, now)
      ...
      if (next === current || next === EMPTY_ASCVD_RISK_INPUTS) return state
  ...
  clearInputs: (patientId) => {
    if (!patientId) return
    discardEncryptedAnswers(ascvdRiskInputsStorageKey(patientId))
    set((state) => ({
      byPatientId: { ...state.byPatientId, [patientId]: EMPTY_ASCVD_RISK_INPUTS },
...
export function useAscvdRiskInputs(patientId: string | undefined): AscvdRiskInputs | undefined {
  return useAscvdRiskInputsStore((state) => (patientId ? state.byPatientId[patientId] : undefined))
}

/** Whether this chart's typed values have been read back yet. */
export function useAscvdRiskInputsHydrated(patientId: string | undefined): boolean {
  return useAscvdRiskInputsStore(
    (state) => !patientId || Boolean(state.hydratedPatientIds[patientId]),
  )
}

/** A record built from one statement, for a caller with no store. */
export function buildAscvdRiskInputs(patch: AscvdRiskInputsPatch, now: Date = new Date()): AscvdRiskInputs {
  return mergeAscvdRiskInputs(EMPTY_ASCVD_RISK_INPUTS, patch, now)
}
