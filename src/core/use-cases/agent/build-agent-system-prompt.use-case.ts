...
3. Never infer a medication ingredient, drug class, indication, formulation, or treatment target from a brand name. Use an ingredient only when terminologyStatus is "matched". Use a class only from the same matched object's medicationIdentity / terminology fields, or from that row's sourceAtcClassification when terminologyStatus is "source-atc-only". Never transfer fields between rows or choose one ambiguous candidate. If the tool did not provide a field, write ${missingValueText}.
