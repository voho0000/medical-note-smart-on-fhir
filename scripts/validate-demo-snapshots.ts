...
// dropped, or the emphasis guardrails would demote the snapshot's highlights.
...
async function main() {
  ...
  for (const aud of ['medical', 'patient'] as const) {
    ...
    console.log(`✓ summary[${aud}]: ${finalized.summary.length} segs (${emph.length} highlights), ${finalized.problems.length} problems, ${finalized.decisions.length} decisions, ${finalized.timeline.length} timeline, ${finalized.sourceIndex.length} sources all verified`)
    ...
    console.log(`✓ safety[${aud}]: ${scan.alerts.length} alerts, all source keys resolve`)
  }
