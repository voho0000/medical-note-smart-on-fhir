...
for(const file of ['page.tsx','measurement-control.tsx'])await fs.copyFile(path.join(here,file+'.template'),path.join(out,'app',file))
await fs.mkdir(path.join(out,'app/visual'),{recursive:true})
await fs.copyFile(path.join(here,'visual.tsx.template'),path.join(out,'app/visual/page.tsx'))
