...
describe('AF preview uses the same encrypted visit data as the live feature', () => {
  ...
  it('waits for reload hydration and keeps measurements and decisions with their patient', async () => {
    ...
    await waitFor(() => expect(screen.getByTestId('cdss-af-inputs')).toHaveTextContent('62 kg'))
    expect(screen.getByRole('combobox', { name: '血栓風險：處置' })).toHaveValue('reviewed')

    fireEvent.change(screen.getByRole('combobox', { name: '案例' }), { target: { value: 'apixaban' } })
    await waitFor(() => expect(screen.getByTestId('cdss-af-inputs')).toHaveTextContent('55 kg'))
    expect(within(screen.getByTestId('cdss-af-inputs')).queryByText(/62 kg/)).not.toBeInTheDocument()
    expect(screen.getByRole('combobox', { name: '血栓風險：處置' })).toHaveValue('')

    fireEvent.change(screen.getByRole('combobox', { name: '案例' }), { target: { value: 'untreated' } })
    await waitFor(() => expect(screen.getByTestId('cdss-af-inputs')).toHaveTextContent('62 kg'))
    expect(screen.getByRole('combobox', { name: '血栓風險：處置' })).toHaveValue('reviewed')
  })
})
