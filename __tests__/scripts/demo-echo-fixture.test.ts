...
describe('curated demo echo fixture', () => {
  it('ships the two linked source procedures as one complete shared report card', () => {
    ...
    expect(new Set(reports.map((report) => report.encounter.reference))).toEqual(new Set(['Encounter/demo-encounter-42']))
    expect(new Set(reports.map((report) => report.effectiveDateTime))).toHaveProperty('size', 1)
  ...
  it('adds the pair without mutating existing demo resources and is idempotent', () => {
    ...
    expect(first.addedResources).toHaveLength(2)
    expect(JSON.stringify(originalResources)).toBe(before)
    expect(first.anchorEncounterId).toBe('demo-encounter-42')
  ...
  it('retains the curated pair on its original encounter when a newer hospital visit appears', () => {
    ...
    expect(result.anchorEncounterId).toBe('demo-encounter-42')
