...
describe('modular summary generation contract', () => {
  ...
  it('builds a card-specific output contract instead of requiring the full summary object', () => {
    ...
    expect(messages[0].content).toContain('"milestones":')
    expect(messages[0].content).toContain('"threads":')
...
describe('finalizeResult — timeline v2 (milestones + care threads)', () => {
  const V2_INPUT = {
    encounters: [
      {
        id: 'v-adm1',
        period: { start: '2026-05-03T10:00:00+08:00', end: '2026-05-08T10:00:00+08:00' },
        class: { code: 'IMP' },
        reasonCode: [{ text: 'K920 吐血', coding: [{ code: 'K920' }] }],
        serviceProvider: { display: '長庚' },
      },
      {
        id: 'v-er1',
        period: { start: '2026-05-02T22:00:00+08:00' },
        class: { code: 'EMER' },
        reasonCode: [{ text: 'K920 吐血', coding: [{ code: 'K920' }] }],
        serviceProvider: { display: '長庚' },
      },
      {
        id: 'v-ckd1',
        period: { start: '2026-04-01T09:00:00+08:00' },
        class: { code: 'AMB' },
        reasonCode: [{ text: 'N183 慢性腎臟病', coding: [{ code: 'N183' }] }],
        serviceProvider: { display: '北港' },
      },
      {
        // NHI dispensing row — an AMB encounter at a 藥局 must never count as
        // a thread visit.
        id: 'v-phar',
        period: { start: '2026-03-15T09:00:00+08:00' },
        class: { code: 'AMB' },
        reasonCode: [{ coding: [{ code: 'N183' }] }],
        serviceProvider: { display: '安心藥局' },
      },
      {
        id: 'v-ckd2',
        period: { start: '2026-02-01T09:00:00+08:00' },
        class: { code: 'AMB' },
        reasonCode: [{ text: 'N1832 CKD3b', coding: [{ code: 'N1832' }] }],
        serviceProvider: { display: '長庚' },
      },
      {
        id: 'v-one',
        period: { start: '2026-01-20T09:00:00+08:00' },
        class: { code: 'AMB' },
        reasonCode: [{ coding: [{ code: 'H612' }] }],
        serviceProvider: { display: '耳鼻喉科' },
      },
      {
        // CKD as SECONDARY reason — prefix matching must scan all reasonCodes.
        id: 'v-ckd3',
        period: { start: '2025-12-01T09:00:00+08:00' },
        class: { code: 'AMB' },
        reasonCode: [{ coding: [{ code: 'I10' }] }, { text: 'N183', coding: [{ code: 'N183' }] }],
        serviceProvider: { display: '北港' },
      },
      {
        id: 'v-adm2',
        period: { start: '2025-01-10T00:00:00+08:00', end: '2025-01-14T00:00:00+08:00' },
        class: { code: 'IMP' },
        reasonCode: [{ text: 'J189 肺炎', coding: [{ code: 'J189' }] }],
        serviceProvider: { display: '北港' },
      },
    ],
    carePlans: [
      { id: 'v-cp1', period: { start: '2024-06-13' }, title: 'Pre-ESRD 照護計畫', author: { display: '北港' } },
    ],
  }
  // Key order (date desc): E1=v-adm1 E2=v-er1 E3=v-ckd1 E4=v-phar E5=v-ckd2
  // E6=v-one E7=v-ckd3 E8=v-adm2, K1=v-cp1.
  const catalog = buildSourceCatalog(V2_INPUT)
  const base = { headline: 'h', summary: [{ text: 't', emphasis: false, sources: [] }], problems: [], decisions: [], timeline: [] }

  it('resolves multi-ref milestones with encounter-derived spans and enforces anchor coverage', () => {
    const ai = {
      ...base,
      milestones: [
        { refs: ['E2', 'E1'], category: 'admission', label: '吐血急診入院', note: '急診當日收治' },
      ],
      threads: [],
    }
    const result = useCase.finalizeResult(ai, catalog)
    ...
    const merged = result.milestones!.find((event) => event.keys.includes('E1'))!
    // Span = min start → max end across the ENCOUNTER refs only.
    ...
    expect(result.milestones!.map((event) => event.date)).toEqual(
      [...result.milestones!.map((event) => event.date)].sort().reverse(),
    )
  })

  it('expands thread rules deterministically: prefix match on any reason code, pharmacy rows excluded, one thread per visit, <2 members dropped', () => {
    const ai = {
      ...base,
      milestones: [
        { refs: ['E1', 'E2'], category: 'admission', label: '吐血住院' },
        { refs: ['E8'], category: 'admission', label: '肺炎住院' },
        { refs: ['K1'], category: 'careplan', label: 'Pre-ESRD 收案' },
      ],
      threads: [
        { label: 'CKD 追蹤', codePrefixes: ['N18'], organizations: ['北港', '長庚'], insight: '跨院追蹤', status: 'active' },
        { label: '重複規則', codePrefixes: ['N18'], organizations: [] },
        { label: '單次就診', codePrefixes: ['H61'], organizations: [] },
      ],
    }
    const result = useCase.finalizeResult(ai, catalog)

    expect(result.careThreads).toHaveLength(1)
    const thread = result.careThreads![0]
    ...
    // Pharmacy dispensing row excluded; secondary-code visit included.
    expect(thread.visits.map((visit) => visit.resourceId)).toEqual(['v-ckd3', 'v-ckd2', 'v-ckd1'])
    expect(thread.organizations).toEqual([
      { name: '北港', count: 2 },
      { name: '長庚', count: 1 },
    ])
  })

  it('maps legacy single-ref picks through the v2 card shape and still enforces coverage', () => {
    const ai = {
      ...base,
      timeline: [{ ref: 'E8', label: '肺炎住院', category: 'encounter' }],
    }
    const result = useCase.finalizeResult(ai, catalog)
  ...
  it('computes deterministic timeline stats from the catalog', () => {
    const result = useCase.finalizeResult({ ...base, milestones: [], threads: [] }, catalog)
    expect(result.timelineStats).toMatchObject({
      start: '2025-01-10',
      end: '2026-05-08',
      admissions: 2,
      emergencies: 1,
