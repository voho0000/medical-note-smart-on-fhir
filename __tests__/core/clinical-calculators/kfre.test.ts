...
describe('four-variable KFRE domain calculator', () => {
  // This file used to assert that the app and `@voho0000/personalized-care`
  // held one implementation, because the equation had once been forked and the
  // two reported different risks for one patient. That package now ships the
  // heart-failure pathway alone and carries neither the CKD guidance card nor
  // this equation, so there is no second implementation to disagree with and
  // the model is vendored here. The worked cases below are what guards it.
