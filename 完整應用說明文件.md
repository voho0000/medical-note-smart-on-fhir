# 醫析 MediPrisma · SMART on FHIR 完整應用說明文件
...
**醫析 MediPrisma · SMART on FHIR** 是一個智能臨床文件助理系統，基於國際醫療資訊標準 FHIR（Fast Healthcare Interoperability Resources）和 SMART on FHIR 授權框架開發。本系統採用**整潔架構（Clean Architecture）**和**可插拔設計（Pluggable Architecture）**，整合先進的人工智慧技術，協助醫療人員：
...
本應用程式由臺北榮民總醫院醫療人工智慧發展中心開發團隊建構，結合臨床實務經驗與現代資訊技術，旨在提升醫療人員的工作效率與決策品質。
...
**GitHub Repository：** https://github.com/voho0000/medical-note-smart-on-fhir
...
- **Literature Search**：使用 Perplexity API 搜尋醫學文獻（需要 Perplexity API 金鑰）
...
#### AI 偏好設定
- **模型選擇**：支援多種 AI 模型
  - 內建模型（無需個人金鑰）：GPT-4o-mini、GPT-4o、Gemini 2.0 Flash、Gemini 2.0 Flash Thinking
  - 進階模型（需個人金鑰）：GPT-4o、GPT-o1、Gemini 2.0 Pro 等
- **API 金鑰管理**：
...
| AI 整合 | Vercel AI SDK 6.0.6（支援 Tool Calling）<br>OpenAI API（GPT-4o, GPT-4o-mini 等）<br>Google Gemini API（Gemini 2.0 Flash, Pro 等）<br>Perplexity API（sonar, sonar-pro） |
...
3. AI 會自動：
   - 判斷需要查詢哪些資料
   - 調用對應的 FHIR Tools 或 Literature Search
   - 整合查詢結果
   - 生成回答並提供引用來源
4. 複製回應內容到病歷系統
...
**範例問題**：
- 一般模式：「請根據病患資料撰寫入院病歷」
- 深入模式：「這個病人有什麼慢性病？請查詢診斷記錄」
- 深入模式：「高血壓的最新治療指引是什麼？」（會自動搜尋文獻）
...
- ✅ 客戶端執行 Tool Calling，避免 Token 外洩
- ✅ Perplexity API 金鑰僅用於文獻搜尋，不涉及病患資料
