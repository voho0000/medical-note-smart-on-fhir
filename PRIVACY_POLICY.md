...
使用者也可以設定院內／地端 OpenAI-compatible endpoint。選用該模型時，提示、所選臨床內容與回應由瀏覽器直接送往該 endpoint，不經 MediPrisma owner-funded proxy；該院內服務的保存、logging、權限與法規責任由實際部署醫療機構決定。
...
院內 OpenAI-compatible Base URL、model id 與 optional key 遵循相同的 session-first／remember-on-device 選擇；key 以既有 browser-side encryption 保存，不會同步至 MediPrisma Firebase。
...
| 自訂院內 AI | 使用者／醫療機構設定的 OpenAI-compatible endpoint | 使用者選擇院內模型時直接處理 AI 請求 |
