...
- `getActiveMedicationList` 只有在 lifecycle status 與可計算的 supply window 都允許時才列入 `data`。缺少或為 `unknown` 的來源狀態放在 `uncertainData`，並令 `canConcludeAbsence=false`，不得被模型敘述成確定現用藥或確定無用藥。
