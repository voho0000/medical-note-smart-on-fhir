...
- 院內模型模式會額外停用 Firestore chat auto-save／smart title、移除 Perplexity tool，並把 follow-up suggestions 與語音轉錄留在相同 endpoint；endpoint unavailable 時不 fallback 到 owner proxy。
