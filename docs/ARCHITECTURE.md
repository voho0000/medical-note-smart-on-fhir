...
另有固定 logical id `openai-compatible-custom`。使用者設定的 HTTPS Base URL、upstream model id 與 optional key 存在 browser config；factory 遇到此 id 時永遠建立 direct OpenAI Chat Completions provider，不進 proxy interceptor。串流、非串流與 Agent 共用此規則，cache identity 則加入 endpoint／model fingerprint，避免不同院內模型共用舊結果。
