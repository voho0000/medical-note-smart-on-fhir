...
一般雲端 provider key 預設加密存於 `sessionStorage`，使用者可切換為 `localStorage`。自訂 connections 則以 localStorage v2 envelope 跨重開保存，每個 profile 的 key 各自加密且與 profile metadata 原子更新。模型偏好與 key 分開儲存，各 AI surface 有自己的 model slot。
